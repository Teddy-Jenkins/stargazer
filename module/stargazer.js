...
Hooks.once('init', async function() {
  ...
  CONFIG.Actor.entityClass = StargazerActor;
  CONFIG.Item.entityClass = StargazerItem;
...
Hooks.once("ready", async function() {
  ...
  Hooks.on("hotbarDrop", (bar, data, slot) => createStargazerMacro(data, slot));
});
 ...
 * @returns {Promise}
 */
async function createStargazerMacro(data, slot) {
  if (data.type !== "Item") return;
  if (!("data" in data)) return ui.notifications.warn("You can only create macro buttons for owned Items");
  const item = data.data;

  // Create the macro command
  const command = `game.stargazer.rollItemMacro("${item.name}");`;
  ...
  if (!macro) {
    macro = await Macro.create({
      ...
      img: item.img,
      command: command,
      flags: { "stargazer.itemMacro": true }
    });
  }
